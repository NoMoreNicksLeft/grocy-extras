...
# Usage: This should be put into a weekly cron job, probably on Saturday. Script
...
# Storage: These circular pdfs seem to weigh in at 30+ megs each. Prepare 
# 150m/month (2 gigs per year) for archiving.

################################################################################
#################################### main() ####################################
################################################################################

r = requests.get(f"https://api.flipp.com/flyerkit/v4.0/publications/cvspharmacy?access_token=d005d9bbce4a77a9f3908b83e38aae19&locale=en-US&store_code={store}")
...
for c in filtered:
    ...
    pdf = f"{file_location}CVS Pharmacy Circular - {date}.pdf"
