...
PAGE='https://dam.flippenterprise.net/flyerkit/publications/homedepotusa?locale=en&access_token=4c1ca51353684ca4e164176a2363d7bf&show_storefronts=true&store_code='
CURL=`which curl`
JQ=`which jq`

QUERY1=".[] | select(.name==\"Weekly Ad\") | .pdf_url"
QUERY2=".[] | select(.name==\"Weekly Ad\") | .valid_from | .[0:10]"

JSON=`$CURL -s $PAGE$STORE`
PDF=`echo $JSON | $JQ -r "$QUERY1"`
DATE=`echo $JSON | $JQ -r "$QUERY2"`

$CURL -s $PDF > "${FILE_LOCATION}Home Depot Circular - $DATE.pdf"