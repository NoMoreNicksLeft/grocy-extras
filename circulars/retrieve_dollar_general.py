...
r = requests.get(f"https://dam.flippenterprise.net/flyerkit/publications/dollargeneral?locale=en&access_token=00be606cd7cb8b0cf999e3c7b038a2fe&show_storefronts=true&store_code={store}")
...
filtered = [j for j in r.json() if j.get('name') == 'Dollar General Weekly Ad']

for c in filtered:
    url = c.get('pdf_url')
    date = c.get('valid_from')[:10]
    pdf = f"{file_location}Dollar General Circular - {date}.pdf"
